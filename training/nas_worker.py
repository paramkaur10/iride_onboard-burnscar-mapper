...
os.environ["CUDA_VISIBLE_DEVICES"] = str(args.gpu)   # BEFORE any torch import
...
CLASS_WEIGHTS = torch.tensor([0.05,0.25,0.35,0.12,0.12,0.11], dtype=torch.float32)
...
pop = Population(n_individuals=len(population), max_layers=MAX_LAYERS, dm=LightDM(),
                 max_parameters=MAX_PARAMS, save_directory=SAVE_DIR)
pop.population, pop.cfg, pop.generation = population, config, args.gen

dm = BalancedTileDataModule(TILES_ROOT, INDEX_CSV, batch_size=BATCH_SIZE, num_workers=2,
                            nas_subset_n=(5_000 if args.dataset=="nas" else None))
dm.setup()

gen_dir = f"{SAVE_DIR}/generation_{args.gen}/model_{args.idx}"
os.makedirs(gen_dir, exist_ok=True)
loss_fn = nn.CrossEntropyLoss(weight=CLASS_WEIGHTS.cuda(), ignore_index=-1)
...
if not valid:
    json.dump({"gen":args.gen,"idx":args.idx,"fitness":0.0,"error":"invalid"},
               open(f"{gen_dir}/metrics.json","w")); sys.exit(0)

LM = GenericLightningSegmentationNetwork(model=model, learning_rate=LR)
LM.loss_fn = loss_fn
trainer = pl.Trainer(max_epochs=FP32_EPOCHS, accelerator="gpu", devices=1,
                     logger=False, enable_checkpointing=False,
                     enable_model_summary=False, log_every_n_steps=50)
trainer.fit(LM, dm)
...
trainer_ft = pl.Trainer(max_epochs=FP16_EPOCHS, accelerator="gpu", devices=1,
                        logger=False, enable_checkpointing=False,
                        enable_model_summary=False, log_every_n_steps=50)
