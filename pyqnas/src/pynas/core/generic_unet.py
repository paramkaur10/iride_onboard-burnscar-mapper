...
def _load_config():
    ...
    cfg_path = os.environ.get("PYNAS_CONFIG")
    if cfg_path and Path(cfg_path).exists():
        loaded = cfg.read(cfg_path)
    else:
        loaded = cfg.read((Path(__file__).with_name("config.ini")).as_posix())
    if not loaded or not cfg.sections():
        raise FileNotFoundError(f"config.ini not found or empty. Tried: {cfg_path or Path(__file__).with_name('config.ini')}")
    return cfg
...
class UNetDecoder(nn.Module):
    ...
    def __init__(self, encoder_shapes, num_classes=2, output_shape=None):
        ...
        self._skip_shape_guard_for_fx = False
    ...
    def forward(self, encoder_features, verbose=False):
        ...
        for i in range(self.num_stages):
            ...
            if not self._skip_shape_guard_for_fx:
